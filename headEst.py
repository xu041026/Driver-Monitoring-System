...
logger = logging.getLogger("paddlehub")
logging.basicConfig(
    level=logging.INFO,  # 你可以改成 DEBUG/ERROR 等
...
class HeadPostEstimation(object):
    ...
    def classify_pose_in_euler_angles(self, datapack, poses=3):
        ...
        if(abs(pitch)>abs(yaw)):
            ...
            if abs(yaw) >= 0.6:
                datapack.isShaking = True
                cv2.putText(datapack.dstFrame, "Lateral head is detected", (5, 380), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)    
        # if yaw >=
        cv2.imwrite("./images/head.jpg", datapack.dstFrame)
        return datapack

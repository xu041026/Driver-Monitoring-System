...
class MainUI:
    def __init__(self):
        ...
        self.keepShaking = False
        self.keepBlinking = False
        self.keepYawning = False
        self.keepFreeing = False
        self.keepSmoking = False
        self.keepPlayPhone = False
        self.keepDrinking = False
        self.shakeFrames = 0
        self.yawnFrames = 0
    ...
    def send_image_background(self, code, image_dir, image_path):
        send_image.send_image(code, image_dir, image_path)
    ...
    def logicProcess(self):
        ...
        if self.nodFlag:
            ...
            if self.datapack.isNodding:
                self.nodFrames += 1
                if not self.keepNodding:
                    self.ui.textResult.append(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + ' 低头警告!')
                    self.nod += 1
                    message = "2"
                    client_socket.send(message.encode())
                    thread = threading.Thread(target=self.send_image_background, args=("1003", "./images/", "head.jpg"))
                    thread.start()
            self.keepNodding = self.datapack.isNodding
            # if self.shakeFlag:
            if self.datapack.isShaking:
                self.shakeFrames += 1
                if not self.keepShaking:
                    # print("yes")
                    self.ui.textResult.append(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + '未正视前方警告!')
                    message = "2"
                    client_socket.send(message.encode())
                    thread = threading.Thread(target=self.send_image_background, args=("1003", "./images/", "head.jpg"))
                    thread.start()
                    self.shake += 1
            self.keepShaking = self.datapack.isShaking

        # 分心行为检测
        if self.yawnFlag:
            ret, self.datapack = myframe.yolo_detect(self.datapack)
            for i in ret:
                if i == "phone":
                    if self.datapack.isPlayPhone:
                        self.playphoneFrames += 1
                        if not self.keepPlayPhone:
                            # print("yes")
                            message = "2"
                            client_socket.send(message.encode())
                            self.ui.textResult.append(
                                time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + '玩手机警告！')
                            thread = threading.Thread(target=self.send_image_background,
                                                      args=("1006", "./images/", "phone.jpg"))
                            thread.start()
                            self.playphone += 1
                    self.keepPlayPhone = self.datapack.isPlayPhone
                if i == "smoke":
                    if self.datapack.isSmoking:
                        self.smokeFrames += 1
                        if not self.keepSmoking:
                            # print("yes")
                            message = "2"
                            client_socket.send(message.encode())
                            self.ui.textResult.append(
                                time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + '抽烟警告！')
                            thread = threading.Thread(target=self.send_image_background,
                                                      args=("1006", "./images/", "smoke.jpg"))
                            thread.start()
                            self.smoke += 1
                    self.keepSmoking = self.datapack.isSmoking
                if i == "drink":
                    if self.datapack.isDrinking:
                        self.drinkFrames += 1
                        if not self.keepDrinking:
                            # print("yes")
                            message = "2"
                            client_socket.send(message.encode())
                            self.ui.textResult.append(
                                time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + '喝水警告！')
                            thread = threading.Thread(target=self.send_image_background,
                                                      args=("1006", "./images/", "drink.jpg"))
                            thread.start()
                            self.drink += 1
                    self.keepDrinking = self.datapack.isDrinking
        ...
        if self.blinkFlag:
            ...
            if self.datapack.isBlinking:
                ...
                if not self.keepBlinking:
                    ...
                    if self.datapack.isSleeping:
                        message = "3"
                        client_socket.send(message.encode())
                        self.ui.textResult.append(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()) + ' 入睡警告!请立即停车休息！')
                    thread = threading.Thread(target=self.send_image_background, args=("1005", "./images/", "eye.jpg"))
                    thread.start()
                    self.blink += 1
            self.keepBlinking = self.datapack.isBlinking
            self.datapack = self.yawnDetector.yawnDet(self.datapack)
            if self.datapack.isYawning:
                ...
                if not self.keepYawning:
                    ...
                    self.yawn += 1
                    thread = threading.Thread(target=self.send_image_background, args=("1005", "./images/", "yawn.jpg"))
                    thread.start()
            self.keepYawning = self.datapack.isYawning
        ...
        if self.handsFlag:
            ...
            if self.datapack.isFreeing:
                ...
                if not self.keepFreeing:
                    ...
                    self.handsfree += 1
                    # thread = threading.Thread(target=send_image, args=("1004","./images/","hand.jpg"))
                    thread = threading.Thread(target=self.send_image_background, args=("1004", "./images/", "hand.jpg"))
                    thread.start()
                    # send_image.send_image("1004","./images/","hand.jpg")
            self.keepFreeing = self.datapack.isFreeing

        self.datapack.timeStamp = time.time()
        if not (
                self.datapack.isFreeing or self.datapack.isYawning or self.datapack.isNodding or self.datapack.isBlinking):
            self.normalFrames += 1

        self.totalFrames += 1

        # 直接获取图像宽高，减少重复操作
        height, width = self.datapack.dstFrame.shape[:2]
        qImg2Show = QImage(self.datapack.dstFrame.data, width, height, QImage.Format_BGR888)
        self.ui.lblCamera.setPixmap(QPixmap.fromImage(qImg2Show))
